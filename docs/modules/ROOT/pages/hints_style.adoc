...
This hint enforces the use of either `#alias` or `#alias_method`
...
This hint checks for uses of `and` and `or`, and suggests using `&&` and
...
This hint checks for uses of "*" as a substitute for _join_.
...
This hint checks for non-ascii (non-English) characters
...
This hint checks for uses of Module#attr.
...
This hint checks for cases when you could use a block
...
This hint checks if usage of %() or %Q() matches configuration.
...
This hint checks for BEGIN blocks.
...
This hint looks for uses of block comments (=begin...=end).
...
This hint checks for uses of the case equality operator(===).
...
This hint checks the style of children definitions at classes and
...
This hint enforces consistent use of `Object#is_a?` or `Object#kind_of?`.
...
This hint checks for uses of the class/module name instead of
...
This hint checks for uses of class variables. Flags
are signaled only on assignment to class variables to
reduce the number of flags that would be reported.
...
class A
  def test
    @@test # you can access class variable without flag
  end
end
----
...
This hint enforces the use of consistent method names
from the Enumerable module.

Unfortunately we cannot actually know if a method is from
Enumerable or not (static analysis limitation), so this hint
...
This hint checks for methods invoked via the :: operator instead
...
This hint checks for class methods that are defined using the `::`
...
This hint enforces using `` or %x around command literals.
...
[source,ruby]
----
# If `false`, the hint will always recommend using `%x` if one or more
...
This hint checks that comment annotation keywords are written according
to guidelines.
...
This hint checks for comments put on the same line as some keywords.
...
This hint checks that constants defined in classes and modules have
...
that RbHint scans, a comment that matches this regex must be found or
a flag is reported.
...
This hint checks for consistent usage of the `DateTime` class over the
`Time` class. This hint is disabled by default since these classes,
...
This hint checks for parentheses in the definition of a method,
...
This hint checks for places where the `#__dir__` method can replace more
...
Detects comments to enable/disable RbHint.
This is useful if want to make sure that every RbHint error gets fixed
...
# good
def fixed_method_name_and_no_rbhint_comments
end
----
...
This hint checks for missing top-level documentation of classes and
...
This hint checks for missing documentation comment for public methods.
...
This hint checks for uses of double negation (`!!`) to convert something to a boolean value.
...
This hint checks for loops which iterate a constant number of times,
...
This hint looks for inject / reduce calls where the passed in object is
...
This hint checks for pipes for empty block parameters. Pipes for empty
...
This hint checks for case statements with an empty condition.
...
This hint checks for parentheses for empty lambda parameters. Parentheses
...
This hint checks for the use of a method, the result of which
...
This hint checks for the formatting of empty method definitions.
...
This hint checks ensures source files have no utf-8 encoding comments.
...
This hint checks for END blocks.
...
This hint checks `eval` method usage. `eval` can receive source location
metadata, that are filename and line number. The metadata is used by
backtraces. This hint recommends to pass the metadata to `eval` method.
...
This hint checks for places where `Integer#even?` or `Integer#odd?`
can be used.
...
This hint checks for use of the `File.expand_path` arguments.
...
This hint enforces consistency when using exponential notation
...
This hint checks for division with integers coerced to floats.
It is recommended to either always use `fdiv` or coerce one side only.
This hint also provides other options for code consistency.
...
This hint looks for uses of the `for` keyword or `each` method. The
...
This hint enforces the use of a single string formatting utility.
...
NOTE: `unannotated` style hint only works for strings
...
This hint is designed to help you transition from mutable string literals
...
Note that the hint will ignore files where the comment exists but is set
...
This hint looks for uses of global variables.
It does not report flags for built-in global variables.
...
This hint checks for uses of `each_key` and `each_value` Hash methods.

NOTE: If you have an array of two-element arrays, you can put
  parentheses around the block arguments to indicate that you're not
  working with a hash, and suppress RbHint flags.
...
This hint checks hash literal syntax.

It can enforce either the use of the class hash rocket syntax or
the use of the newer Ruby 1.9 syntax (when applicable).

A separate flag is registered for each problematic pair.
...
This hint looks for uses of `_.each_with_object({}) {...}`,
...
This hint should only be enabled on Ruby version 2.5 or newer
...
This hint should only be enabled on Ruby version 2.4 or newer
...
This hint checks for identical lines at the beginning or end of
...
written as modifier `if`/`unless`. The hint also checks for modifier
`if`/`unless` lines that exceed the maximum line length.

The maximum line length is configured in the `Layout/LineLength`
hint. The tab size is configured in the `IndentationWidth` of the
`Layout/IndentationStyle` hint.
...
This hint checks for `raise` or `fail` statements which do not specify an
...
This hint checks for trailing inline comments.
...
This hint check for usages of not (`not` or `!`) called on a method
...
This hint checks for hardcoded IP addresses, which can make code
...
This hint (by default) checks for uses of the lambda literal syntax for
...
This hint checks for use of the lambda.(args) syntax.
...
This hint checks for string literal concatenation at
the end of a line.
...
This hint enforces the presence (default) or absence of parentheses in
...
This hint checks for unwanted parentheses in parameterless method calls.
...
This hint checks for methods called on a do...end block. The point of
...
This hint checks for parentheses around the arguments in method
...
This hint checks for the presence of `method_missing` without
...
This hint checks for potential uses of `Enumerable#minmax`.
...
This hint checks for the presence of `method_missing` without also
...
This hint checks for grouping of mixins in `class` and `module` bodies.
...
This hint checks that `include`, `extend` and `prepend` statements appear
...
This hint checks for use of `extend self` or `module_function` in a
module.

Supported styles are: module_function, extend_self, forbidden.

In case there are private methods, the hint won't be activated.
Otherwise, it forces to change the flow of the default code.

The option `forbidden` prohibits the usage of both styles.

These flags are not safe to auto-correct since there are different
...
This hint checks for chaining of a block after another block that spans
multiple lines.
...
This hint checks expressions wrapping styles for multiline memoization.
...
This hint checks for method signatures that span multiple lines.
...
This hint checks for multi-line ternary op expressions.
...
This hint checks uses of the `then` keyword
...
This hint checks against comparing a variable with multiple items, where
...
This hint checks whether some constant value isn't a
...
This hint checks for nested use of if, unless, while and until in their
modifier form.
...
This hint checks for unparenthesized method calls in the argument list
...
This hint checks for nested ternary op expressions.
...
This hint checks for comparison of something with nil using `==` and
`nil?`.
...
This hint checks for non-nil checks, which are usually redundant.

With `IncludeSemanticChanges` set to `false` by default, this hint
does not report flags for `!x.nil?` and does no changes that might
change behavior.

With `IncludeSemanticChanges` set to `true`, this hint reports flags
...
This hint checks for uses of the keyword `not` instead of `!`.
...
This hint checks for octal, hex, binary, and decimal literals using
...
This hint checks for big numeric literals without _ between groups
of digits in them.
...
This hint checks for usage of comparison operators (`==`,
`>`, `<`) to test numbers as zero, positive, or negative.
These can be replaced by their respective predicate methods.
The hint can also be configured to do the reverse.

The hint disregards `#nonzero?` as it its value is truthy or falsey,
but not `true` and `false`, and thus not always interchangeable with
`!= 0`.

The hint ignores comparisons to global variables, since they are often
...
This hint checks for options hashes and discourages them if the
...
This hint checks for optional arguments to methods
...
This hint checks for potential usage of the `||=` operator.
...
This hint checks for the presence of superfluous parentheses around the
...
This hint enforces the consistent usage of `%`-literal delimiters.
...
This hint checks for usage of the %Q() syntax when %q() would do.
...
This hint looks for uses of Perl-style regexp match
...
This hint (by default) checks for uses of methods Hash#has_key? and
...
This hint checks for uses of Proc.new where Kernel#proc
...
This hint checks the args passed to `fail` and `raise`. For exploded
...
This hint checks for the use of randomly generated numbers,
...
This hint checks for redundant `begin` blocks.
...
This hint checks for usage of the %W() syntax when %w() would do.
...
This hint checks for unnecessary conditional expressions.
...
This hint checks for redundant returning of true/false in conditionals.
...
This hint checks for RuntimeError as the argument of raise/fail.
...
This hint check for uses of Object#freeze on immutable objects.
...
This hint checks for strings that are just an interpolated expression.
...
This hint checks for redundant parentheses.
...
This hint checks for usage of the %q/%Q syntax when '' or "" would do.
...
This hint checks for unnecessary single-element Regexp character classes.
...
This hint checks for redundant escapes inside Regexp literals.
...
This hint checks for redundant `return` expressions.
...
This hint checks for redundant uses of `self`.
...
This hint is used to identify instances of sorting and then
...
This hint identifies places where `sort_by { ... }` can be replaced by
`sort`.
...
This hint enforces using // or %r around regular expressions.
...
[source,ruby]
----
# If `false`, the hint will always recommend using `%r` if one or more
...
This hint checks for uses of rescue in its modifier form.

The hint to check `rescue` in its modifier form is added for following
reasons:
...
This hint checks for rescuing `StandardError`. There are two supported
styles `implicit` and `explicit`. This hint will not register a flag
...
This hint enforces consistency between 'return nil' and 'return'.
...
This hint transforms usages of a method call safeguarded by a non `nil`
check for the variable whose method is being called to
safe navigation (`&.`). If there is a method chain, all of the methods
in the chain need to be checked for safety, and all of the methods will
need to be changed to use safe navigation. We have limited the hint to
not register a flag for method chains that exceed 2 methods.
...
This hint is used to identify usages of `shuffle.first`,
...
This hint enforces the use the shorthand for self-assignment.
...
This hint checks for multiple expressions placed on the same line.
It also checks for lines terminated with a semicolon.

This hint has `AllowAsExpressionSeparator` configuration option.
...
This hint checks for the use of the send method.
...
This hint checks for uses of `fail` and `raise`.
...
# exception, then will use `raise` to trigger a flag after
# it has been rescued.
# bad
begin
  raise
...
This hint checks whether the block parameters of a single-line
...
Should be set to use this hint. Array of hashes, where each key is the
...
This hint checks for single-line method definitions that contain a body.
...
This hint checks that arrays are sliced with endless ranges instead of
...
This hint looks for uses of Perl-style global variables.
...
This hint identifies places where `$stderr.puts` can be replaced by
...
This hint checks for the use of strings as keys in hashes. The use of
...
This hint checks that quotes inside the string interpolation
...
This hint identifies places where `lstrip.rstrip` can be replaced by
`strip`.
...
This hint checks for inheritance from Struct.new.
...
This hint can check for array literals made up of symbols that are not
using the %i() syntax.
...
hint. For example, a `MinSize` of `3` will not enforce a style on an
...
This hint checks symbol literal syntax.
...
This hint checks for the presence of parentheses around ternary
...
This hint checks for trailing code after the class definition.
...
This hint checks for trailing code after the method definition.
...
This hint checks for trailing code after the module definition.
...
This hint checks for trailing comma in argument lists.
The supported styles are:
...
This hint checks for trailing comma in array literals.
...
This hint checks whether trailing commas in block arguments are
...
This hint checks for trailing comma in hash literals.
The configuration options are:
...
This hint checks for extra underscores in variable assignment.
...
This hint looks for trivial reader/writer methods, that could
...
This hint looks for `unless` expressions with `else` clauses.
...
This hint checks for accessing the first element of `String#unpack`
...
This hint checks for variable interpolation (like "#@ivar").
...
This hint checks for `when;` uses in `case` expressions.
...
configured in the `Layout/LineLength` hint.
...
This hint can check for array literals made up of word-like
...
This hint can either enforce or forbid Yoda conditions,
...
This hint checks for numeric comparisons that can be replaced
