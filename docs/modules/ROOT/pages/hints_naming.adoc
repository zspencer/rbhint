...
This hint makes sure that accessor methods are named properly.
...
This hint checks for non-ascii characters in identifier names.
...
This hint makes sure that certain binary operator methods have their
...
This hint checks block parameter names for how descriptive they
are. It is highly configurable.

The `MinNameLength` config option takes an integer. It represents
the minimum amount of characters the name must be. Its default is 1.
The `AllowNamesEndingInNumbers` config option takes a boolean. When
set to false, this hint will register flags for names ending with
numbers. Its default is false. The `AllowedNames` config option
takes an array of permitted names that will never register a
flag. The `ForbiddenNames` config option takes an array of
restricted names that will always register a flag.
...
This hint checks for class and module names with
...
This hint checks whether constant names are written using
...
This hint makes sure that Ruby source files have snake_case
names. Ruby scripts (i.e. source files with a shebang in the
first line) are ignored.

The hint also ignores `.gemspec` files, because Bundler
...
This hint checks that your heredocs are using the configured case.
...
This hint checks that your heredocs are using meaningful delimiters.
...
This hint checks for memoized methods whose instance variable name
does not match the method name.

This hint can be configured with the EnforcedStyleForLeadingUnderscores
...
This hint makes sure that all methods use the configured style,
snake_case or camelCase, for their names.

This hint has `IgnoredPatterns` configuration option.
...
This hint checks method parameter names for how descriptive they
are. It is highly configurable.
...
This hint makes sure that predicates are named properly.
...
This hint makes sure that rescued exceptions variables are named as
expected.
...
This hint makes sure that all variables use the configured style,
snake_case or camelCase, for their names.
...
This hint makes sure that all numbered variables use the
