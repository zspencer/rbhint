...
It's possible to extend RbHint with custom hints and formatters.
...
You can configure the custom hints in your `.rubocop.yml` just like any
other hint.
...
See xref:development.adoc[development] to learn how to implement a hint.
...
Custom hints and config defaults for Chef Infra Cookbooks
...
You can customize RbHint's output format with custom formatters.
...
You can tell RbHint to use your custom formatter with a combination of
...
[source,sh]
----
$ rbhint --require ./path/to/my_custom_formatter --format MyCustomFormatter
----
